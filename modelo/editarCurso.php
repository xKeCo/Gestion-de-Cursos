...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $consultaEditar = mysqli_query($link, "SELECT * FROM curso WHERE id =$id");
}else{
    $id= $_POST['id'];
...
if(empty($fecha_de_inicio_alerta) && empty($Horario_alerta) && empty($nombre_instructor_alerta) && empty($correo_instructor_alerta) && $puede_enviar == true){
    ...
    $sql = "UPDATE curso SET usuario=?, fechaInicio=?, Horario=?, nombreInstructor=?, correoInstructor=? WHERE id=$id";
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if(mysqli_stmt_execute($stmt)){
            $fecha_de_inicio_alerta2 = "Curso editado correctamente";
